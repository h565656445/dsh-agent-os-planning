...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/agent-os-planning/`：

```powershell
# Windows PowerShell 7
$dst = Join-Path $env:DSH_HOME ".agent-presets\agent-os-planning"
Copy-Item -Recurse -Force ".\dsh" $dst
```

重启 DeepSeek Harness 后，在会话预设列表选择 `Agent OS 规划 Loop`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-agent-os-planning/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-agent-os-planning"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\skills\dsh-agent-os-planning\SKILL.md" $dst -Force
```
